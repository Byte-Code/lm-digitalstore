...
export default function productReducer(state = Map(), action) {
  ...
  switch (type) {
    ...
    case 'SUCCESS_FETCH_RELATEDPRODUCTS':
      return state.setIn([productCode, 'similarProducts', groupCode], result.getIn(['content', 'itemlist']));
    case 'SUCCESS_FETCH_XSELLPRODUCTS':
      return state.setIn([productCode, 'similarProducts', 'POTREBBE SERVIRTI ANCHE'], result.getIn(['content', 'itemlist']));
